...
// import "./proxy/utils/0.7/Initializable.sol";
import "./IAmaENSClient.sol";


contract TestAmaClient is  ChainlinkClient, AmaCLClientStorage{
    using Chainlink for Chainlink.Request;
     uint256 constant PAYMENT = 1 * 10 ** 16;
     bytes public GLOBALDATA;
    ...
    event DifferentSubDomainAllowed(
     ...
     constructor()  {
        ...
        setChainlinkOracle(0x5730Eb302331a965b42a84898f9d240fb58dAEe9);
        ensContract = ENS(0xB7E62FbB5F0AC4FEaCDF1394Cdc7933081B87483);
        amaENSclientContract = IAmaENSClient(0x69b92B7e01b81EaCD8E3F37203d08B923A0D5198);
        specId = bytes32("d819128d289a45e5a391f02651793095");
    ...
    modifier ethAddressVerified(){
        bytes32 _ethaddressHash = keccak256(abi.encodePacked(msg.sender));
        require(!results[_ethaddressHash].verifiedOnChain, "Eth address has already been verified");
        _;
    }
    ...
    function requestVerification(string calldata _twitterUsername) 
        ...
        bytes32 _ethaddressHashByte32 = keccak256(abi.encodePacked(msg.sender));
    // 	Chainlink.Request memory req = buildOperatorRequest(specId, this.fulfillBytes.selector);
        ...
        bytes32 _reqID = requestOracleData(req, PAYMENT);

        // bytes32  _reqID =  requestOracleData(req, PAYMENT);

    	results[_ethaddressHashByte32].reqID = _reqID;
    	results[_ethaddressHashByte32].twitterUsername = _twitterUsername;

    	addressRequestIDs[_reqID] = _ethaddressHashByte32;
    	return _reqID;
    }
    ...
    function fulfillBytes(bytes32 _requestId, 
    	   ...
    	   //require(msg.sender == oracle, "Only operator can fullfill the request");
            GLOBALDATA = bytesData;
    	   bytes32 _ethaddressHash =  addressRequestIDs[_requestId];

    	   if (bytesData.length >10 ){
    	   
        	   results[_ethaddressHash].verifiedOnChain = true;
        	   results[_ethaddressHash].data = bytesData;
        ...
        // results[_ethaddressHash].onchainUsername = proposedUsername;
    	   ...
    	   //emit RequestErrored(bytes(results[_ethaddressHash].ethaddress),  bytesData);
    }
    ...
    */
    function claimSubDomain() 
        ...
        Response memory _response = results[_ethaddressHashByte32];
        ...
        */
        ...
        _setLabel(_ethaddressHashByte32, label);
    ...
    function claimSubDomain( 
        ...
        (,string memory nameOnTwitter, string memory profileImage, string memory twitterID, bool isTwitterVerified, bool verified) = 
        decode_data(_response.data);

        bytes memory _new = abi.encodePacked(_differentLabel,nameOnTwitter,profileImage,twitterID,isTwitterVerified,verified);
        (bytes32 nodehash, ) = _registerNode(msg.sender, _new, _response.twitterUsername);


        _setLabel(_ethaddressHashByte32, _differentLabel);
    ...
    */
    ...
    function _setLabel(bytes32 _ethaddressHashBytes32, string memory _label) private {
        results[_ethaddressHashBytes32].label = _label;
    ...
    function userDetailsTwitter(address _address) 
        ...
        returns (string memory, string memory, string memory, string memory, bool){
            bytes32 _ethaddressHashByte32 = keccak256(abi.encodePacked(_address));
            Response memory _response = results[_ethaddressHashByte32];
            require(_response.verifiedOnChain, "Address is not yet verified onChain");
    ...
    */
    function getLabel(address _address) 
        ...
        bytes32 _ethaddressHashByte32 = keccak256(abi.encodePacked(_address));
        bytes32 _requestId = results[_ethaddressHashByte32].reqID;
        _verificationStarted(_requestId);
        if (isPending(_requestId) == true) {
            return results[_ethaddressHashByte32].label; 
        }
        return "";
    ...
    function getEncodedData() 
        ...
        _verificationStarted(_requestId);
        if (isPending(_requestId) == true) {
            return results[_ethaddressHashByte32].data; 
        }
        return "";
    }
